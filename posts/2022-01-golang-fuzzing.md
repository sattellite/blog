...
то его данные добавляются в корпус в директорию `testdata`. Так же результат
показывает команду, с помощью которой можно вызывать конкретный набор данных
вызывавших ошибку в функции.

Мой пример тестов слишком упрощенный, но в блоге go есть полный пример 
нахождения ошибки и ее дальнейшего исправления - https://go.dev/doc/tutorial/fuzz
